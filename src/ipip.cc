...
ipip_spider_ip::ipip_spider_ip(std::string ip_b, std::string ip_e)
{
    ...
    memcpy(User_Agent,"User-Agent: Mozilla/5.0 (Windows NT 6.1; \
        ...
        WOW64; rv:52.0) Gecko/20100101 Firefox/52.0"));
    memcpy(url, "www.ipip.com/ips138.asp?ip=", strlen("www.ipip.com/ips138.asp?ip=")); 
    char file_name[64] = {0};
...
std::string ipip_spider_ip::confirm_url(std::string ip)
{
    if(once_flag == 0)
    {
        ...
        std::string new_url = current_ip1; 
        once_flag = 1;
        pre_ip = {ip};
        return new_url;
    }
...
void ipip_spider_ip::curl_http()
{
    ...
    while(1)
    {
        ...
        sprintf(post_data,"ip=%s",(char*)(current_url.data()));
...
int ipip_spider_ip::cidr(std::string ip_s, std::string ip_e, std::string des) 
{
